  ...
  return (
    <PopoverTrigger asChild>
      <Button
        ...
        className={cn(
          "flex h-auto min-h-9 w-fit items-center justify-between gap-2 overflow-hidden rounded-md border border-input bg-transparent px-3 py-1.5 text-sm whitespace-nowrap shadow-xs transition-[color,box-shadow] outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 data-[placeholder]:text-tertiary dark:bg-input/30 dark:hover:bg-input/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 [&_svg:not([class*='text-'])]:text-tertiary",
          className
        )}>
        {children}
...
export function MultiSelectValue({
  ...
  if (selectedValues.size === 0 && placeholder) {
    return (
      <span className="min-w-0 overflow-hidden default-font-family font-normal text-tertiary">
        {placeholder}
      </span>
    );
  }
      ...
      {[...selectedValues]
        ...
        .map(value => (
          <Badge
            ...
            {items.get(value)}
            {clickToRemove && (
              <XIcon className="size-2 text-tertiary group-hover:text-destructive" />
            )}
          </Badge>
        ))}
      <Badge
